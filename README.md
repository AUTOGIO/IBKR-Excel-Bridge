...
**Run:** `./scripts/setup.zsh` once, then open TWS/Gateway (paper, Read-Only API), then `./scripts/run.zsh` or `./scripts/refresh_workbook.command` (closes Excel, snapshots, reopens). Drop statement CSVs into `data/statements/` and use `./scripts/ingest.zsh` / `./scripts/promote_events.zsh` when needed.
